...
| `TEMPER_MCP_SERVICE_SECRET` | api, mcp | Yes | The relay's service-to-service credential — the API trusts the `X-Temper-Relayed-Surface: mcp` attribution carrier ONLY beside a constant-time match against this value. The **same** generated value on both functions (`openssl rand -base64 32`); whitespace-trimmed at parse on both sides, so a secret-file trailing newline does not silently degrade the fleet to `@web`. The API refuses to boot if it overlaps any other shared secret it holds (the distinctness gate), or if it is under 16 characters — a paste placeholder or a committed test constant. Unset on the API, every relayed act attributes `@web` and the trust gate is simply never armed |
