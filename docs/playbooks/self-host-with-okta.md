...
- **A base Temper deployment.** This page replaces only the "Provision Auth0" and "Configure
  the CLI" sections of [Self-hosting Temper](./self-host-temper.md). Everything outside auth —
...
- **The auth-identity contract.** Temper validates every request's JWT against an issuer, an
  ...
  switch. Read [Auth identity](../concepts/auth-identity.md) for the contract this page
...
Okta-issued tokens validate. See [Auth identity](../concepts/auth-identity.md) for the full
contract.

Two consequences shape the rest of this page:

1. Temper requires a **custom audience** on its access tokens. Okta can only mint custom
   audiences from a **custom authorization server**, which requires the **API Access
   Management** add-on.
2. Okta's authorization-server URLs differ from Auth0's (an issuer with no trailing slash;
...
audience** (`aud`) claim, and its access tokens are intended for Okta's own APIs — not for
...
enabled before continuing.** Without it, there is no supported way to host Temper on Okta.
...
2. **Add Rule** — in the rule's grant-type conditions, allow **Authorization Code** and
...
[base deployment's table](./self-host-temper.md#environment-variable-contract); only the auth
values differ.
...
Use the same verification steps as the [base deployment](./self-host-temper.md#verify):
...
- **Native SAML SSO as an alternative to an OIDC bridge:**
  [Self-host with SAML](./self-host-with-saml.md).
