...
A [cognitive map](https://temperkb.io/cognitive-maps/what-a-cognitive-map-is) is a
...
- **Temper installed and authenticated.** If you have not installed the CLI, see
  [install Temper](../playbooks/install-temper.md). You must be authenticated to author.
  ...
  [authoring authorization](../concepts/authoring-authorization.md).
- **A context holding your source material.** A map cites resources by id regardless of which
  [context](../concepts/contexts-and-refs.md) homes them, but you need citable sources
...
[what lives in a map](https://temperkb.io/cognitive-maps/what-lives-in-a-map).
...
its formation-delta threshold, so run it at the end of every pass. Verify with `temper cogmap
shape <MAP>`: multi-member, high-cohesion regions forming around your concept nodes is the
...
the earlier ones.** [The substrate](https://temperkb.io/cognitive-maps/the-substrate-beneath-it)
...
→ why it breaks → what's settled about it* in one connected structure. For how maps relate to
each other and to their contexts, see
[how maps relate](https://temperkb.io/cognitive-maps/how-maps-relate).
...
into `managed_meta` by the server**, derived from the act envelope. Do not pass it by hand.
...
4. `invocation open`.
5. Author node #1. Show it. Calibrate voice with whoever the map is for.
