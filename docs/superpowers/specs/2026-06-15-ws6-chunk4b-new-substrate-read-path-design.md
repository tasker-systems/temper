...
- **Invariant (asserted equal across `legacy` and `next`):** `origin_uri`, `title`, `is_active`, `context_name`, `doc_type_name`, `stage`, `mode`, `effort`, `seq`.
- **Non-invariant (NOT asserted):** `id`, `kb_context_id`, `owner_profile_id`, `originator_profile_id` (re-minted); `slug`, `managed_hash`, `open_hash` (§7-dissolved); `created`, `updated` (synthesis-collapsed); `owner_handle` (production projects it caller-relative — `"@me"` when owner == caller — while the substrate carries the raw handle; a render-time decoration); `body_hash` (production stores the manifest hash, `temper_next` recomputes a chunk merkle — different algorithms; the §8 gate already compares reconstructed body TEXT, not the hash columns).
