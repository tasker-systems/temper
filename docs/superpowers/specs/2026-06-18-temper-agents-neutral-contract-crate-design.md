...
Create `crates/temper-agents` — the **neutral-contract package** for temper's agent surface (WS7 decision #6). It is a deliberately **thin layer**: it re-exports the invocation-envelope and agent-authorship contract types already shipped in `temper-next` (PR #148), and *owns* exactly one new thing — the `DeploymentProfile` policy object plus its ts-rs wire export. It replaces the dead `temper-llm` / `temper-llm-smoke` crates, which are culled. The thin MCP/HTTP client wrapper named in decision #6 is **rejected** (not deferred): agentic runtimes reach temper over the `temper-mcp` URL surface or the `temper` CLI binary, so `temper-agents` is a contract crate — an interface declaration for how flows are *invoked and traced* — never a client for invoking them. See *Rejected* below.
...
- **A thin MCP/HTTP client wrapper inside `temper-agents`** (originally WS7 decision #6 — raised by the author, retracted here on 2026-06-19). Was *deferred*; now **rejected** outright, superseded by the runtime investigation. Both reference runtimes reach temper as an *external resource*, not a linked library: they bind tools by **MCP-over-URL** (Eve `defineMcpClientConnection({url})`, CMA `mcp_servers: [{type:"url"}]`) and ship managed credential stores (Vercel Connect; CMA Vaults `environment_variable`, substituted at egress). So an agentic flow touches temper through exactly two seams — (i) the `temper-mcp` URL surface, or (ii) the `temper` CLI binary running in a sandbox with injected secrets — and **neither links a Rust client into the contract crate**. `temper-client` has exactly one consumer in the workspace (`temper-cli`); pulling it into `temper-agents` would re-couple the neutral contract to the local HTTP write path it deliberately excludes — the same rationale as the `EventContext` / `fire_with` rejection above. `temper-agents` stays an *interface declaration* (envelope/authorship types + `DeploymentProfile`) for how flows are invoked and traced, not a means of invoking them. **Corollary scope (the real next thread):** making "trace" actually reachable is server-side work — an `invocation_open` / `invocation_closed` MCP tool that writes the envelope through the existing backend → service path — not a client in this crate.
...
- Realizes WS7 decision #6 (`temper-llm → temper-agents`, neutral contract) and grounds decision #7 (deployment-profile shape) in the runtime investigation. **Retracts** the thin-MCP/HTTP-client-wrapper clause of decision #6 (see *Rejected*); the charter's decision #6 carries a matching scar.
