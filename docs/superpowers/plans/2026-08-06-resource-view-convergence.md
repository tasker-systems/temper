...
- **⚠️ THE PRE-COMMIT HOOK IS NOT A SUPERSET OF `cargo make check`** `[found — 2026-08-06, after Task 1]`. The hook runs fmt, clippy, docs and `check-openapi-spec.sh`. It does **not** run `ts-rs-drift`, so a `ts_rs::TS`-derived type can be committed with no regenerated binding and a green hook — which is exactly what Task 1 did with `ResourceView`, and it would have failed CI. **A task is not done when the hook passes; it is done when `cargo make check` passes.** Run the full gate before considering any task complete.
...
**Interfaces:**
- Produces: `create_resource`, `update_resource`, `annotate_resource`, `show_resource` all return `CommandOutput<ResourceView>`.
- ⚠️ **`list_resources` and `search_resources` are DELETED from the trait, not converted** `[corrected — 2026-08-06, before Task 6]`. This plan said `list_resources` returns `CommandOutput<Vec<ResourceView>>`; that predates the finding. Re-verified on disk: **neither method has a single call site anywhere in the workspace** — every occurrence is the declaration, the two `CloudBackend` stubs, the `db_backend` impl, or a comment (`rg '\.\s*list_resources\s*\(|\.\s*search_resources\s*\('` over `crates/` + `tests/` returns only two error-message *string literals*). `CloudBackend`'s bodies said so out loud: *"not implemented — reads stay surface-direct"*. They are vestigial surface from before the reads-stay-service-direct decision that repo CLAUDE.md now states as policy, so converting them would have minted a shape nobody consumes. Deleting them is what actually retires `ResourceSummary` and `SearchHit`.
...
- ⚠️ **The trait boundary and the wire boundary move in different commits, so something sits between them for one commit** `[found — 2026-08-06, during Task 6]`. `/api/resources`, `/api/ingest`, the MCP tools and the CLI all still speak `ResourceRow`/`ResourceDetail` until Task 7 / Beat D. So Task 5's private `view_to_row` / `view_to_detail` moved to `temper-workflow`'s `types/resource.rs` as `impl From<ResourceView> for ResourceRow` / `for ResourceDetail` — **unchanged in body, still "deleted by Task 7"** — because temper-api, temper-mcp AND temper-cli now all narrow, and **temper-cli does not depend on temper-services**. A third, `impl From<ResourceRow> for ResourceView`, is new: `CloudBackend` receives a row from `temper-client` and must hand back the view the trait declares. All three go in Task 7. `show_detail_select` survives — `temper-mcp`'s `get_resource` still calls it directly (`tools/resources.rs:758`), so it is Task 7's/Task 9's to remove, not Task 6's.
