...
> **Status (2026-07-04):** Phase A shipped (#259), Phase B shipped (#261). Phase C is itself split: **Tasks 9–10 shipped** on branch `jct/t7c-remote-sources` (the `'remote'` source kind — `kb_remote_sources` + `normalize_remote_uri`/`_upsert_remote_source` + projector remote branch + URL read surfacing; and URL sources on `--sources` across CLI/MCP via the shared `resolve_provenance_source` classifier). **Tasks 11–12 remain** as follow-up task `019f2e46` (per-content-block `--content-block` addressing + steward agent wiring).
