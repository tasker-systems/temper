...
> **Status (2026-07-04):** Phase A shipped (#259), Phase B shipped (#261). Phase C is itself split: **Tasks 9–10 shipped** (#263 — the `'remote'` source kind: `kb_remote_sources` + `normalize_remote_uri`/`_upsert_remote_source` + projector remote branch + URL read surfacing; and URL sources on `--sources` across CLI/MCP via the shared `resolve_provenance_source` classifier). **Task 11 shipped** on branch `jct/t7c-content-block-addressing` (per-content-block `--content-block <uuid>` addressing: surface-only threading of `content_block: Option<Uuid>` down the `sources` spine — `BodyUpdate` → `ResourceUpdateRequest` → API handler → `DbBackend` → `writes::UpdateParams` — overriding the single-block selection in `update_resource_in_tx` with belongs-to-resource + non-folded validation; also the escape hatch for multi-block resources. CLI `--content-block` + MCP `content_block` at parity). **Task 12 shipped** (#266 — steward agent wiring: the distillation loop's `create_resource` now carries `sources` alongside the `derived_from` edge; per-source nodes pass their one source id, synthesized nodes pass every id they distill from, external sources go as raw http(s) URLs; prompt/skill-only change to the Eve agent).
>
> **T7 COMPLETE — verified end-to-end on live cogmap `019f2391` (2026-07-05).** Both halves of Task 12's acceptance criterion proven against production: (1) `kb_block_provenance` rows exist — every distilled node checked carries source rows (synthesized nodes carry multiple, accreted at `accretion_seq` 0/1); ticks at 00:02–01:03 UTC all recorded provenance; (2) after `cogmap_materialize` (258 formation events → 57 regions), region `reference_standing` reflects them — salience distributed 1–8 across regions by how referenced each region's nodes are. Remote (`source_uri`) column present; these ticks distilled resource refs so all `null`, wiring exercised via CLI e2e `remote_url_source_round_trips_through_cli_api_db`.
