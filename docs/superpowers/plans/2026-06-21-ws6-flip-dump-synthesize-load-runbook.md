...
# untouched), 01/02 install schema+functions. This is the SAME set the synthesis
# tests' reset_artifact() loads (tests/common/mod.rs) — deliberately WITHOUT 03_seed.
# 03_seed.sql is a demo scenario fixture (the alice/bob onboarding-cogmap for
# 04_scenarios.sql), NOT system seed; its 20 demo resources have no public counterpart,
# so loading it makes the §8 body-parity gate flag all of them. Synthesis seeds the
# system actors it needs itself (bootstrap::run).
