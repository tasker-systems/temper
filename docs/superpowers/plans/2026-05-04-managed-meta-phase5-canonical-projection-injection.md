...
> | 7 — Full quality bar | `7b8d756` | `cargo make check`, `cargo make test` (1015 / 1015), `cargo make test-db` (163 / 163 in temper-api), `cargo make test-e2e` (114 / 114), `cargo sqlx prepare --workspace --check` clean |
> | (extension) — symmetric receive-side defense on `meta_service` | `4bc6aa8` | After review the asymmetric-defense gap (PUT `/api/resources/{id}/meta` lacked the helper + relied on caller-computed hashes) was closed for consistency. `meta_service::update_meta` now reads current title/slug/doc_type, runs `ensure_managed_identity_keys` over the typed-`ManagedMeta`-serialized JSONB, and recomputes `managed_hash` / `open_hash` server-side. `payload.managed_hash` / `payload.open_hash` become advisory; manifest stores what the server computes. New test `meta_service_update_meta_preserves_temper_title_in_jsonb` asserts the load-bearing case (PUT with `title: None` typed payload still preserves stored `temper-title`). Two e2e tests (`mcp_round_trip::mcp_update_resource_meta_preserves_chunks_and_body_hash`, `meta_test::meta_patch_preserves_chunks_and_body_hash`) updated for the new server-recompute semantics — they previously asserted manifest hash matched payload hash; they now assert the hash advanced and is a server-canonical sha256. |
> | (Phase 1 follow-up) — refresh fixture-hash goldens | `dbd9176` | Pre-existing bug surfaced during Phase 5's full quality bar: Phase 1's drop of `date` from session/research managed-tier left `session_full` and `research_full` managed-hash goldens stale in `tests/frontmatter_test.rs` and the inline session fixture in `src/frontmatter/tiers.rs`. Refreshed; refactored `fixture_hashes_match_goldens` to report all drifts in one shot for future schema migrations. |
>
> **Symmetric defense, all three write surfaces:**
>
> | Surface | Send-side | Receive-side | Hash source |
> |---|---|---|---|
> | `POST /api/ingest` (`ingest_service::ingest`) | CLI `build_ingest_payload` + MCP `create_resource` (Tasks 3 / 4) | `ensure_managed_identity_keys` after defaults, before validate (Task 5) | server recompute |
> | `PATCH /api/resources/{id}` (`resource_service::update`) | CLI cloud-mode update (option 2 — defers to receive-side, commented), MCP `update_resource` (Tasks 3 / 4) | `ensure_managed_identity_keys` after merge, before hash; gate extended to fire on title/slug-only PATCHes (Task 5) | server recompute |
> | `PUT /api/resources/{id}/meta` (`meta_service::update_meta`) | CLI sync flush + MCP `update_resource_meta` (typed `ManagedMeta` is canonical-by-construction post-Phase-1 serde renames) | `ensure_managed_identity_keys` after typed-to-Value serialization, with title/slug/doc_type resolved from typed-payload-wins / current-row-fallback (extension `4bc6aa8`) | server recompute |
>
> **Branch state:** `jct/wave1-shared-execution-paths-and-cloud-first-reframe`, 27 commits ahead of main. 10 commits this phase: 8 plan-tracked (`ee1599e`, `84747e2`, `d7170f9`, `2992551`, `329b5c0`, `2be0629`, `2f92b7f`, `7b8d756`) plus the extension (`4bc6aa8`) and the Phase 1 goldens follow-up (`dbd9176`). Working tree clean.
>
