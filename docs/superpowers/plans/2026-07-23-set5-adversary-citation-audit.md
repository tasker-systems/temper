...
- [ ] **Step 2: Write the migration.**
  ...
  - **`standing_band`** — DROP the old `(double precision,int,double precision,double precision,double precision)` signature, CREATE the new `(int,int,double precision,double precision,double precision)` with **[review] four arms** (spec §3.1), coverage-ratio-gated and **calibrated for one-pass reachability** (spec §3.1 "Calibrated for the system's real dynamics"):
    - `near-canonical`: `citation_magnitude >= 2 AND audit_coverage = citation_magnitude AND citation_quality > 0.3 AND contradiction_balance >= 0.0` (magnitude floor **2**, the Landmesser line, not 3; full coverage, which one thorough pass reaches).
    - `reinforced`: `citation_magnitude >= 1 AND audit_coverage::float / citation_magnitude >= 0.5 AND citation_quality > 0.0 AND contradiction_balance >= 0.0`
    - `disputed`: `audit_coverage > 0 AND citation_quality < 0.0`
    - else `provisional` (includes every unaudited finding: coverage 0 ⇒ falls through here).

    Guard the ratio against `magnitude = 0` (an unaudited or citation-less finding must land `provisional`, never divide-by-zero). **The `citation_quality > 0.3` cut is a provisional low default, NOT a fixed threshold** (spec §3.1): the absolute number is meaningful only relative to the auditor's scoring distribution, which does not exist until Task 13, so it is co-calibrated with the auditor prompt then and re-tuning it later is a one-migration change with no backfill. Do not treat `0.3` as load-bearing — the load-bearing parts are the magnitude floor of 2 and the full-coverage requirement.
...
- [ ] **Step 3: Prune the orphaned tests, then write the new ones.** **[review]** Four existing tests in `evidential_standing.rs` call dropped objects and must be **deleted** (not left to fail): `silence_default_is_correlated` (`:204`), `affirmed_independence_raises_breadth` (`:227`, the only writer of `'independent-of'`), `zero_challenges_is_not_survival` (`:287`), `band_is_read_time_over_components` (`:311`, passes the old 5-arg `standing_band` signature). Then add:
  ...
  8. **[review]** `near_canonical_is_reachable_in_one_pass` — build a finding with **2** distinct live resource-kind citations and audit **both** positively in a single sequence (no `supports` edge, since nothing writes those; `contradiction_balance` stays at its default `0.0`, which satisfies the `>= 0.0` conjunct). Assert the band is `near-canonical`. This proves the recalibration (spec §3.1): the top band is reachable through the ordinary audit path at magnitude 2 with full coverage, without repeated rounds and without a `supports`-edge writer. If this test cannot be made to pass, the thresholds are still too high — fix the thresholds, not the test.
  9. `one_source_never_reaches_near_canonical` — a single citation audited `+1.0` lands `reinforced`, never `near-canonical` (the Landmesser floor: one source is not diverse evidence, spec §3.1).
...
- [ ] **Step 5: Write the session prompt.** Direct the agent to: read the finding's citations via `get_block_provenance`; pull `element_trail` **only** for the citing acts it weighs (discrete calls, never a bulk pass — spec §8); weigh the citing act's confidence/rationale, related resources, and citation-set size (spec §3.3); emit a signed verdict per **resource-kind** citation via the Task 9 tool with its **own** authorship; and — verbatim — *"assess only whether the source carries the connection claimed; never whether the claim is true, and never what the source says"* (spec §Bedrock, §3.4).
  ...
  > **[review] Co-calibrate the auditor's use of the `[-1,1]` range with Task 3's `citation_quality > 0.3` band cut** (spec §3.1). The band threshold is meaningful only relative to how the prompt tells the auditor to score — e.g. reserve the top of the range for "the source fully carries the claimed connection," the middle for "adequately," negative for "does not." Once the prompt fixes that scale, revisit Task 3's `standing_band` quality cut against it (it is a one-migration change, no backfill) so the top band means what the scale says it means. Record the chosen scale in the prompt so the threshold and the scoring stay coupled.
