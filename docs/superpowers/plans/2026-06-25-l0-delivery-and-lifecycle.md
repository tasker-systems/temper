...
6. **Facet model (settled Task 2→3 — two distinct mechanisms; do not conflate).** The substrate has two property shapes:
   - **Clustering facet** — `SeedAction::FacetSet` writes ONE `kb_properties` row with `property_key='facet'` and the whole values object (e.g. `{layer: concept}`). This is what materialization/affinity read (`substrate.rs:68` filters `property_key='facet'`). The **`layer`** landmark-category (concept/invariant/reference/boundary) is a clustering facet and MUST be written this way.
   - **Per-key property** — `SeedAction::PropertyAssert` (append) / `PropertySet` (single-valued, folds prior) write a `kb_properties` row with `property_key=<the key>` (e.g. `property_key='provenance'`). This is what `readback::resource_row`/`meta`/`kernel_slice` read.
   - **`provenance: kernel` is a per-key property the reconciler STAMPS on create** (constant — every resource the kernel reconcile creates is `provenance: kernel` by definition). It is **not** carried per-entry in the wire/manifest. `kernel_slice` filters on it (`property_key='provenance'`).
   - Therefore `ReconcileEntry.facets` carries **clustering facets only** (e.g. `{layer: concept}`) → `FacetSet`. The authored manifest (Task 8) is byte-identical to the workbench seed's `facets:` maps — no per-entry provenance needed. The applier strips any stray `provenance` key from `entry.facets` before `FacetSet` (provenance is stamped separately, never clustered).
...
- Produces (consumed by Task 4):
  ...
  - `set_facet(pool, resource: ResourceId, values: &serde_json::Value, weight: f64, emitter: EntityId) -> Result<()>` — wraps `SeedAction::FacetSet` (the **clustering** facet, `property_key='facet'`; for `layer`). EXTEND: currently raw in `loader.rs:131`.
  - `set_property(pool, resource: ResourceId, key: &str, value: &serde_json::Value, emitter: EntityId) -> Result<()>` — wraps `SeedAction::PropertySet` (single-valued **per-key** property, folds prior `(owner,key)` then asserts; for `provenance: kernel`). EXTEND: `PropertySet` is raw in `writes.rs:206`. This is the per-key shape `kernel_slice` reads — see Decision #6.
...
4. **Diff entries:** for each incoming `ReconcileEntry`:
   - absent in `live` → **create**: `create_kernel_resource(chunks = Some(unpack(entry.chunks_packed)))`, then **stamp `set_property(rid, "provenance", "kernel")`** (so `kernel_slice` sees it next run — Decision #6), then `set_facet(entry.facets minus any stray "provenance" key)` (clustering, for `layer`), then assert its `edges`; `created += 1`.
