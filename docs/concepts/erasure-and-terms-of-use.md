...
the act's own computation, its strike verdicts simulating the act's own sequential refcount,
so in the same state it matches what the act then records (a commit or strike after the
survey can still flip the strike-time verdict) — and writes nothing: a caller without
