...
//!
//! Drives the production MCP caller path across the network door (`relay_client` → the real
//! `create_app` listener → the API's ingest door) against real Axum + real Postgres, after opening a
//! real invocation envelope through the production client (`app.client.invocations()`). The act-stamp
//! is asserted directly off `kb_events` because the readback `invocation_show` does not yet surface
//! per-act authorship — that exposure is Chunk D, which adds a show-based assertion on top of this
//! vertical.
//!
...
async fn mcp_create_under_invocation_stamps_act_with_authorship(pool: sqlx::PgPool) {
    ...
    temper_mcp::tools::resources::create_resource(&svc, &parts, input)
        .await
