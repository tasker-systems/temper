...
//! `cargo make test-e2e-embed`. The `TEMPER_ASYNC_EMBED` gate is scoped with `temp_env` so it never
//! leaks to a sibling test.
...
async fn deferred_create_is_fts_immediate_then_ready_after_drain(pool: sqlx::PgPool) {
    temp_env::async_with_vars([("TEMPER_ASYNC_EMBED", Some("1"))], async move {
