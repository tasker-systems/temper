 ...
 * `true` — full model message history, exported. The mention agent reads a mentioning
 * user's temper data under that user's own credential, so the pin to `false` is
 * load-bearing and this test is its witness: an agent whose `instrumentation.ts` drops
 * the spread fails here rather than exporting message history.
 */
