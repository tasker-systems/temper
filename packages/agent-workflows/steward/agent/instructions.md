...
derived from the act envelope you already send — so carry `model` on the act and the
trio follows. Supply `managed_meta` explicitly only to override a derived value.
