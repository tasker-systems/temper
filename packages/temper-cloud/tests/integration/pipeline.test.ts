...
import { runOrSkipOnNetworkFlake } from "./helpers/network.js";
...
describe("fixture embedding", () => {
  it("embeds chunked markdown to 768-dim vectors", async (ctx) => {
    ...
    // Pulling the embedding model reaches HuggingFace Hub over the network. A
    // connectivity failure *or* a slow download that hangs past our budget is
    // environment flake, not a defect — degrade to a skip rather than a red.
    const embeddings = await runOrSkipOnNetworkFlake(ctx, "embedding model pull", () =>
      embedTexts(texts),
    );
