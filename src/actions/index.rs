// temper index action — incremental embed pipeline
...
/// Run the incremental embedding pipeline and return index statistics.
///
/// The `on_progress` callback receives human-readable progress messages that
/// the caller can display however it likes (e.g. via `output::progress`).
pub fn run<F>(
    config: &Config,
    force: bool,
    ...
    F: Fn(&str),
{
    ...
    on_progress(&format!("Collecting files: {} total", all_files.len()));
    ...
    for file in &all_files {
        ...
        match registry::compute_file_hash(file) {
            ...
            Err(e) => tracing::warn!("Could not hash {}: {e}", file.display()),
        }
    }
    ...
    let cached_vectors: HashMap<String, Vec<f32>> = if unchanged_paths.is_empty() {
        ...
        match SearchIndex::load(state_dir) {
            ...
            Err(_) => {
                on_progress("  No existing index found; all files will be re-embedded");
                HashMap::new()
            }
        }
    };
    ...
    let mut emb = embedder::Embedder::new(config.model_cache_dir.clone());
    ...
    for (i, rel_path) in files_to_embed.iter().enumerate() {
        ...
        if let Ok(meta) = std::fs::metadata(&full_path) {
            if meta.len() > 1_000_000 {
                on_progress(&format!("  Large file ({}): {}", meta.len(), rel_path));
            }
        }
        ...
        let content = match std::fs::read_to_string(&full_path) {
            Ok(c) => c,
            Err(e) => {
                tracing::warn!("Skipping {rel_path} (read error: {e})");
                continue;
            }
        };

        on_progress(&format!("  [{}/{}] {}", i + 1, total_to_embed, rel_path));
        ...
        match embed_chunks(&mut emb, &chunks) {
            Ok(vecs) => {
            ...
            Err(e) => {
                tracing::warn!("Failed to embed {rel_path}: {e}");
            }
        }
    }
    ...
    let total_documents = files_embedded + files_reused;
    let total_chunks = chunks_embedded + chunks_reused;
...
fn embed_chunks(emb: &mut embedder::Embedder, chunks: &[chunker::Chunk]) -> Result<Vec<Vec<f32>>> {
