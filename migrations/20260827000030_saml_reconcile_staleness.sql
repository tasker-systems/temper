...
--
--   last_skipped_at    -- the assertion carried no group signal, so agreement was not attempted.
--                         `extractGroups` returns null when the IdP has no `groups_attr` configured
--                         or the named attribute is absent from THIS assertion
--                         (packages/temper-cloud/src/saml/sp.ts:63-77), and a null is refused as an
...
COMMENT ON COLUMN kb_saml_principal_reconcile.last_skipped_at IS
  'The moment an assertion for this principal last carried no group signal (no groups_attr '
  'configured, or the attribute absent from that assertion), so agreement was not attempted. Never '
  'written together with last_reconciled_at for the same event: a skip must not read as a success.';
