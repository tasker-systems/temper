...
CREATE TABLE kb_resource_manifests (
    ...
    managed_hash  VARCHAR(64) NOT NULL DEFAULT '',
    open_hash     VARCHAR(64) NOT NULL DEFAULT '',
...
LANGUAGE SQL STABLE AS $$
    ...
    manifest_entries AS (
        SELECT
            ...
            COALESCE(entry->>'remote_body_hash', entry->>'remote_hash', '')::VARCHAR(64) AS remote_body_hash,
            COALESCE(entry->>'managed_hash', '')::VARCHAR(64) AS local_managed_hash,
            COALESCE(entry->>'remote_managed_hash', '')::VARCHAR(64) AS remote_managed_hash,
            COALESCE(entry->>'open_hash', '')::VARCHAR(64) AS local_open_hash,
            COALESCE(entry->>'remote_open_hash', '')::VARCHAR(64) AS remote_open_hash
    ...
    SELECT
        ...
        sr.kb_uri,
        sr.body_hash::VARCHAR(64),
        sr.managed_hash::VARCHAR(64),
        sr.open_hash::VARCHAR(64),
        sr.updated,
        CASE
            ...
            -- Meta changed locally only
            WHEN me.local_managed_hash != me.remote_managed_hash
                 AND sr.managed_hash = me.remote_managed_hash THEN 'to_push_meta'
            WHEN me.local_open_hash != me.remote_open_hash
                 AND sr.open_hash = me.remote_open_hash THEN 'to_push_meta'
            -- Meta changed on server only
            WHEN sr.managed_hash != me.remote_managed_hash
                 AND me.local_managed_hash = me.remote_managed_hash THEN 'to_pull'
            WHEN sr.open_hash != me.remote_open_hash
                 AND me.local_open_hash = me.remote_open_hash THEN 'to_pull'
            -- Meta changed on both sides
            WHEN me.local_managed_hash != me.remote_managed_hash
     ...
     WHERE sr.is_active = false
        ...
        OR me.local_managed_hash != sr.managed_hash
        OR me.local_open_hash != sr.open_hash
    ...
    SELECT
        ...
        me.uri AS kb_uri,
        me.local_body_hash::VARCHAR(64) AS body_hash,
        me.local_managed_hash::VARCHAR(64) AS managed_hash,
        me.local_open_hash::VARCHAR(64) AS open_hash,
...
LANGUAGE SQL STABLE AS $$
    ...
    SELECT r.id AS resource_id,
           r.origin_uri,
           COALESCE(m.body_hash, '')::VARCHAR(64) AS body_hash,
           r.updated,
           r.is_active,
           v.access_level,
           v.team_role
