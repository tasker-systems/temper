-- ============================================================================
-- Temper — canonical baseline schema (migration 1 of 3)
-- ----------------------------------------------------------------------------
-- The single source of truth for the substrate's structure: enums, tables,
-- indexes, and the grafted identity/infra layer. This is a canonical sqlx
-- migration baseline — it replaces the retired 46-file legacy lineage and the
-- former `schema-artifact/` one-shot artifact.
--
...
-- against the connection's search_path. Extensions (`vector`) and the
-- `uuid_generate_v7()` generator are reached via search_path.
--
...
--
-- Out of scope (operational/sync Domain-A tables not central to the cognitive-map
-- model): kb_device_sync_state, kb_resource_search_index (FTS rebuilt by trigger in prod).
-- WS6 GRAFT: the operational identity/infra tables (kb_profile_auth_links, kb_system_settings,
-- kb_join_requests, kb_team_invitations, kb_transfers, kb_blob_files, kb_ingestion_records) are
...
-- ============================================================================
-- UUID v7 GENERATOR (portability prerequisite)
-- ----------------------------------------------------------------------------
...
--   • PG18 (local dev / CI / e2e) → no `pg_uuidv7`; alias the native `uuidv7()`
--     under the single portable name the call sites use.
-- Folded in verbatim from the legacy portability migration's compat block (only the
-- generator block — its legacy chunk-dedup functions reference dropped tables).
DO $uuid_compat$
BEGIN
...
-- pgvector extension prerequisite. The substrate declares `vector(768)` columns
-- (kb_chunks.embedding, kb_cogmap_regions.centroid); the type must exist before
-- those tables are created. Same rationale as the UUID generator: the former
-- `schema-artifact/` assumed `vector` pre-existed in a host `public`, but the
-- canonical baseline IS `public` and self-provisions it (an sqlx-applied
-- `#[sqlx::test(migrations=…)]` / a fresh `sqlx migrate run` has no pre-step).
-- Verbatim from the legacy lineage (20260330000001_consolidated_schema.sql:9).
