...
# here: the entry asserts a posture, and the posture is a property of where the merge lands. A later
...
AUTH_COVERED='auth_only_routes|gated_routes|query_routes|blob_segment_routes'
...
# `blob_segment_routes` is the same shape: a one-route sub-router whose only reason to exist
# is scoping a `DefaultBodyLimit` to the raw-binary segment door (and to nothing else). It
# carries no auth layer of its own and is auth-covered only for as long as `gated_routes`
# is where it is merged.
require_layer_in 'blob_segment_routes()'             gated_routes
