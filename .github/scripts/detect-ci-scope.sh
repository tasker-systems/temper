...
if [ "$SKIP_ALL" = "true" ]; then
    ...
    if [ "$HAS_DOCS_GATED" = "true" ]; then
        ...
        SCOPE_SUMMARY="${SKIP_REASON}: skipping test-rust, test-typescript, test-ruby, test-python, test-agents-ts and the rust-quality job; running code-quality for its pure-bash guard-tests (the docs/ gate)"
    else
