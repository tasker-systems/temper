 ...
 */
export function negotiationKey(raw: string): string | null {
	let parsed: URL;
	try {
		parsed = new URL(raw);
	} catch {
 ...
 */
function writableStatus(span: Span): { status: SpanStatus } {
	return span as { status: SpanStatus };
}
 ...
 */
export class McpNegotiationStatusProcessor implements SpanProcessor {
	 ...
	 */
	private isMandatedNegotiation(span: ReadableSpan): boolean {
		const attrs = span.attributes;
		if (attrs[ATTR_HTTP_REQUEST_METHOD] !== 'GET') return false;
		if (attrs[ATTR_HTTP_RESPONSE_STATUS_CODE] !== 405) return false;
