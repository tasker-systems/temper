...
module Temper::Generated
  ...
  class BlockSuccessor < ApiModelBase
    ...
    # The resource the successor's row lives on — the same value the pair-keyed read fork keys on, so the gated envelope alone constructs the successor's `<home>#<block>` address.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Temper::Generated::BlockSuccessor` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Temper::Generated::BlockSuccessor`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
