...
module Temper::Generated
  # HTTP body for `POST /api/cognitive-maps/{id}/grants` — the subject is the path `{id}` (a cogmap), so the body carries only the principal + capabilities. The handler widens this into a `GrantCapabilityRequest` with `subject_table='kb_cogmaps'`, `subject_id={id}`.
  class CogmapGrantBody < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Temper::Generated::CogmapGrantBody` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Temper::Generated::CogmapGrantBody`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
