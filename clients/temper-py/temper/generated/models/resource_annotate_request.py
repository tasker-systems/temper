...
class ResourceAnnotateRequest(BaseModel):
    """
    ...
    content_block: Optional[UUID] = Field(default=None, description="Which content block to annotate. `None` → the resource's sole non-folded body block; `Some(id)` addresses that block explicitly (must belong to the resource and be non-folded).")
    sources: Optional[List[ProvenanceSource]] = Field(default=None, description="Sources to attach to the addressed block — resource refs, external URLs, or event ids (`{kind,value}`-tagged). Position → accretion `seq`. Must be non-empty.  `ts(skip)`-ped for the same reason as `ResourceUpdateRequest.sources`: `ProvenanceSource` is a tagged enum the SvelteKit UI never sends (this is a CLI/agent write path).")
    __properties: ClassVar[List[str]] = ["confidence", "correlation_id", "invocation_id", "model", "persona", "rationale", "reasoning", "content_block", "sources"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ResourceAnnotateRequest from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ResourceAnnotateRequest from a dict"""
        if obj is None:
            return None
