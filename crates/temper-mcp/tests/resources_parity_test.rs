#![cfg(feature = "test-db")]
//! The resources family's parity suite — the pre-migration binding, pinned.
//!
//! [Beat G3a](temper task `01a0cb1b-42e6-7922-a9dc-b30c402d8a3f`) moves the resources
//! tools' execution from the direct-service path onto temper-client's in-process door.
//! This suite is authored FIRST, against the direct-service binding, and must be green
//! here before the swap lands: a parity suite that never saw the old binding cannot
//! prove parity (`no-door-regression`'s per-family bite — the same tests must stay green
//! with the door underneath, and redden under mutation probes that break any pinned
//! shape, refusal kind, or gate).
//!
//! The tools are driven the way production drives them: a real `TemperMcpService`
//! whose profile was resolved from `Parts` extensions exactly as `require_mcp_auth`
//! injects them, then the `tools::resources` handler functions. Response shapes and
//! refusal kinds asserted below are the incumbent ones, quoted from
//! `crates/temper-mcp/src/tools/resources.rs` at the pre-migration head.
...
mod parity {
    ...
    use temper_mcp::middleware::BearerToken;
    ...
    pub const ISSUER: &str = "https://as.test";
    pub const API_AUD: &str = "https://inst.test/api";
    pub const MCP_AUD: &str = "https://inst.test/mcp";
    const SECRET: &[u8] = b"witness";

    /// A real-DB `AppState` with a static HS256 key: both the MCP service's
    /// `authenticate_token` call and (post-migration) the router's own gate validate
    /// against this one store — the one-trust-domain shape in miniature.
    pub fn test_state(pool: PgPool) -> AppState {
        let config = ApiConfig {
            ...
            auth: AuthConfig {
                ...
                audience: API_AUD.to_string(),
                mcp_audience: MCP_AUD.to_string(),
                mode: AuthMode::ExternalIdp,
            },
            auth_provider_name: "test".to_string(),
            cors_origins: vec![],
            port: 0,
        ...
        let jwks = JwksKeyStore::with_static_key(
    ...
    /// An HS256 bearer naming the MCP audience — the token class a real MCP caller
    /// presents, and one the MCP middleware accepts (`either surface audience`).
    pub fn mint_token(sub: &str, email: &str) -> String {
    ...
    /// `Parts` carrying exactly the two extensions `require_mcp_auth` injects on success
    /// (middleware.rs:98): the decoded claims and the raw bearer.
    pub fn authed_parts(sub: &str, email: &str, token: &str) -> http::request::Parts {
        let (mut parts, _) = http::Request::builder()
            .body(())
            .expect("request builds")
            .into_parts();
        parts.extensions.insert(RawJwtClaims {
            sub: sub.to_string(),
        ...
        parts.extensions.insert(BearerToken(token.to_string()));
        parts
    }

    /// A provisioned, standing-approved caller: a fresh `TemperMcpService` whose profile
    /// was resolved through the service's own seam, plus the email for follow-up SQL.
    pub async fn caller(pool: &PgPool, tag: &str) -> (TemperMcpService, String) {
        let unique = Uuid::new_v4();
        let sub = format!("{tag}-sub-{unique}");
        let email = format!("{tag}-{unique}@example.com");
        let token = mint_token(&sub, &email);

        let svc = TemperMcpService::new(test_state(pool.clone()));
        let parts = authed_parts(&sub, &email, &token);
        // Level 1 of the seam provisions the profile before level 2's standing gate
        // denies it, so the first call's denial IS the provisioning signal — approve,
        // then re-enter the same seam.
        let _ = svc.ensure_profile_from_parts(&parts).await;
        approve_standing_by_email(pool, &email).await;
        svc.ensure_profile_from_parts(&parts)
            .await
            .expect("the profile resolves through the shared seam");
        (svc, email)
    }
    ...
    /// The id of the profile's auto-provisioned default context — the writable home.
    pub async fn default_context_id(pool: &PgPool, email: &str) -> Uuid {
        sqlx::query_scalar(
            "SELECT c.id \
             FROM kb_contexts c \
    ...
    /// Create through the tool under test and hand back the response value.
    pub async fn create(
        ...
        let res = temper_mcp::tools::resources::create_resource(
            svc,
            input(create_body(&ctx, title, content)),
        )
        .await
        .expect("create lands");
        one_text(&res)
...
use parity::{code_of, create, default_context_id, input, one_text, parts_of};
use serde_json::json;
use sqlx::PgPool;
use uuid::Uuid;
...
/// the view's own keys), plus `status: "created"` (resources.rs:730-733 — status is
/// always `Created` on this surface; the backend's dedup converges silently).
#[sqlx::test(migrator = "temper_services::MIGRATOR")]
async fn create_answers_in_the_enriched_shape_with_status_created(pool: PgPool) {
    let (svc, email) = parity::caller(&pool, "create-shape").await;
    let value = create(
        &svc,
        &pool,
        &email,
        "Created through the direct binding",
        None,
    )
    .await;
...
/// The home-anchor refusals are caller errors: both homes, and neither
/// (resources.rs:582-595 — `invalid_params` with the named sentences).
#[sqlx::test(migrator = "temper_services::MIGRATOR")]
async fn create_refuses_both_and_neither_home(pool: PgPool) {
    let (svc, email) = parity::caller(&pool, "create-home").await;
    let ctx = default_context_id(&pool, &email).await;
...
/// Two distinct gate faces on the create path, both `invalid_params`: a context that is
/// not VISIBLE fails resolution with the doubled sentence (resources.rs:619-626 — the
/// tool prefixes "context not found: " onto the service's own "context not found or not
/// readable"), while a context that is visible but not WRITABLE reaches the backend's F1
/// gate and renders the Forbidden arm's sentence (resources.rs:709-715).
#[sqlx::test(migrator = "temper_services::MIGRATOR")]
async fn create_refuses_a_context_without_write_access(pool: PgPool) {
    let (owner, owner_email) = parity::caller(&pool, "create-owner").await;
    let (other, other_email) = parity::caller(&pool, "create-other").await;
    let ctx = default_context_id(&pool, &owner_email).await;
    let other_profile_id: Uuid = sqlx::query_scalar("SELECT id FROM kb_profiles WHERE email = $1")
        .bind(&other_email)
        .fetch_one(&pool)
        .await
        .expect("the other profile");

    // Not visible: resolution fails before any write gate.
    //
    // NAMED PARITY DELTA (beat G3a): the direct binding's tool wrapped the resolver's
    // failure as "context not found: context not found or not readable" — the
    // `build_create_command` prefix doubled the service's own sentence. Through the door
    // the service speaks once: same refusal kind (invalid_params), same gate, one cause
    // named once.
    let err = temper_mcp::tools::resources::create_resource(
        &other,
    ...
    .bind(
        sqlx::query_scalar::<_, Uuid>("SELECT id FROM kb_profiles WHERE email = $1")
            .bind(&owner_email)
            .fetch_one(&pool)
            .await
...
/// An unresolvable context ref is a caller error naming the cause. Through the door the
/// service's own sentence arrives once (see the named parity delta on
/// `create_refuses_a_context_without_write_access` — the direct binding's doubled
/// "context not found: " prefix is gone with `build_create_command`'s wrapper).
#[sqlx::test(migrator = "temper_services::MIGRATOR")]
async fn create_refuses_an_unknown_context(pool: PgPool) {
    let (svc, _email) = parity::caller(&pool, "create-unknown").await;
...
/// leaves the JSON to become its OWN content part (resources.rs:783-805).
#[sqlx::test(migrator = "temper_services::MIGRATOR")]
async fn get_returns_open_meta_and_splits_the_body_into_a_second_part(pool: PgPool) {
    let (svc, email) = parity::caller(&pool, "get-body").await;
    let body = "The body that parity pins.";
    let created = create(&svc, &pool, &email, "Get body split", Some(body)).await;
    let id = created["resource"]["id"].as_str().unwrap();

    let without = one_text(
        &temper_mcp::tools::resources::get_resource(&svc, input(json!({ "id": id })))
            .await
            .expect("get lands"),
    );
...
/// `internal_error` naming the read (resources.rs:488-489) — parity pins what IS,
/// defects and all; improving the kind is a later, deliberate change.
#[sqlx::test(migrator = "temper_services::MIGRATOR")]
async fn get_unknown_resource_is_an_internal_error_naming_the_read(pool: PgPool) {
    let (svc, _email) = parity::caller(&pool, "get-unknown").await;
...
/// `fields` filters TOP-LEVEL keys, anchored on `id` (resources.rs:792-797).
#[sqlx::test(migrator = "temper_services::MIGRATOR")]
async fn get_fields_projection_keeps_the_anchor_and_drops_the_rest(pool: PgPool) {
    let (svc, email) = parity::caller(&pool, "get-fields").await;
    let created = create(&svc, &pool, &email, "Projected", None).await;
    let id = created["resource"]["id"].as_str().unwrap();
...
/// The list envelope carries the paging state the agent skill instructs callers to read
/// (resources.rs:396-428), and rows arrive enriched (embedding_status per row).
#[sqlx::test(migrator = "temper_services::MIGRATOR")]
async fn list_envelope_carries_paging_state_and_enriched_rows(pool: PgPool) {
    let (svc, email) = parity::caller(&pool, "list-env").await;
    let ctx = default_context_id(&pool, &email).await;
    create(&svc, &pool, &email, "Listed one", None).await;
    create(&svc, &pool, &email, "Listed two", None).await;
...
/// (resources.rs:982-984).
#[sqlx::test(migrator = "temper_services::MIGRATOR")]
async fn list_refuses_an_unresolvable_context_filter(pool: PgPool) {
    let (svc, _email) = parity::caller(&pool, "list-unknown").await;
    let ghost = Uuid::now_v7();
...
/// A tag containing a comma is refused, not split (resources.rs:941-948).
#[sqlx::test(migrator = "temper_services::MIGRATOR")]
async fn list_refuses_a_tag_containing_a_comma(pool: PgPool) {
    let (svc, _email) = parity::caller(&pool, "list-tags").await;
...
/// The update response is the ENRICHED current state (resources.rs:1097-1101), and the
/// gates render refusals, not faults: not-found and not-modifiable are distinct
/// `invalid_params` sentences (resources.rs:1080-1091).
#[sqlx::test(migrator = "temper_services::MIGRATOR")]
async fn update_returns_the_enriched_view_and_refuses_gates(pool: PgPool) {
    let (owner, owner_email) = parity::caller(&pool, "upd-owner").await;
    let (other, _other_email) = parity::caller(&pool, "upd-other").await;
    let created = create(&owner, &pool, &owner_email, "Original title", None).await;
...
/// guard fires at the tool (resources.rs:534-541).
#[sqlx::test(migrator = "temper_services::MIGRATOR")]
async fn update_refuses_sources_without_content(pool: PgPool) {
    let (svc, email) = parity::caller(&pool, "upd-sources").await;
    let created = create(&svc, &pool, &email, "No sources yet", None).await;
    let id = created["resource"]["id"].as_str().unwrap();
...
/// Annotate returns the itemized provenance rows it just recorded
/// (resources.rs:1154-1163) — an external URL classifies as a Remote source.
#[sqlx::test(migrator = "temper_services::MIGRATOR")]
async fn annotate_returns_the_provenance_rows_it_recorded(pool: PgPool) {
    let (svc, email) = parity::caller(&pool, "annotate").await;
    let created = create(
        &svc,
        &pool,
        &email,
...
/// The meta-only path answers `{updated: true, id}` (resources.rs:1222-1228) and the
/// write is visible on the next read. The doc type is `task` because the managed
/// vocabulary is doc-type-scoped — `temper-stage` belongs to task/goal, and a session
/// carrying it is refused (`managed_meta inapplicable for doc_type=session`).
#[sqlx::test(migrator = "temper_services::MIGRATOR")]
async fn update_meta_answers_the_ack_and_lands_the_change(pool: PgPool) {
    let (svc, email) = parity::caller(&pool, "meta").await;
    let ctx = default_context_id(&pool, &email).await;
    let mut body = parity::create_body(&ctx, "Meta target", None);
    body["doc_type_name"] = json!("task");
    let created = one_text(
        &temper_mcp::tools::resources::create_resource(&svc, input(body))
            .await
            .expect("task lands"),
    );
    let id = created["resource"]["id"].as_str().unwrap();
...
/// second delete is a `Resource not found:` refusal (resources.rs:1259-1261).
#[sqlx::test(migrator = "temper_services::MIGRATOR")]
async fn delete_answers_the_ack_and_the_resource_stops_answering(pool: PgPool) {
    let (svc, email) = parity::caller(&pool, "delete").await;
    let created = create(&svc, &pool, &email, "Doomed", None).await;
    let id = created["resource"]["id"].as_str().unwrap();

    let ack = one_text(
        &temper_mcp::tools::resources::delete_resource(&svc, input(json!({ "id": id })))
            .await
            .expect("delete lands"),
    );
    assert_eq!(ack, json!({ "deleted": true, "id": json!(id) }));

    let gone = temper_mcp::tools::resources::get_resource(&svc, input(json!({ "id": id })))
        .await
    ...
    // one — a tombstoned row is not-modifiable, not missing (resources.rs:1255-1258).
    let again = temper_mcp::tools::resources::delete_resource(&svc, input(json!({ "id": id })))
        .await
...
/// address renders as data (`state: "absent"`), and a not-visible home renders as a
/// `invalid_params` refusal (resources.rs:832-856).
#[sqlx::test(migrator = "temper_services::MIGRATOR")]
async fn block_read_tri_state_holds(pool: PgPool) {
    let (owner, owner_email) = parity::caller(&pool, "block-owner").await;
    let (other, _other_email) = parity::caller(&pool, "block-other").await;
    let created = create(
        &owner,
        &pool,
        &owner_email,
...
/// The lineage walk is bidirectional over `derived_from` EDGES (resources.rs:863-895;
/// migration 20260712000080 — source = deriver, target = ancestor, label-keyed). The
/// edge is minted explicitly through the relationships tool as the fixture; resource-ref
/// provenance rows do NOT mint lineage edges.
#[sqlx::test(migrator = "temper_services::MIGRATOR")]
async fn lineage_walks_derived_from_both_ways(pool: PgPool) {
    let (svc, email) = parity::caller(&pool, "lineage").await;
    let parent = create(&svc, &pool, &email, "The parent", None).await;
    let parent_id = parent["resource"]["id"].as_str().unwrap().to_string();

    let ctx = default_context_id(&pool, &email).await;
    let child_body = parity::create_body(&ctx, "The child", None);
    let child = one_text(
        &temper_mcp::tools::resources::create_resource(&svc, input(child_body))
            .await
            .expect("child lands"),
    );
    ...
    let lineage = one_text(
        &temper_mcp::tools::resources::resource_lineage(&svc, input(json!({ "id": child_id })))
            .await
    ...
    let reverse = one_text(
        &temper_mcp::tools::resources::resource_lineage(&svc, input(json!({ "id": parent_id })))
            .await
            .expect("lineage lands"),
    );
...
/// with the mapping's sentence (resources.rs:1331-1339); the owner CAN grant.
#[sqlx::test(migrator = "temper_services::MIGRATOR")]
async fn grant_gates_and_outcomes_hold(pool: PgPool) {
    let (owner, owner_email) = parity::caller(&pool, "grant-owner").await;
    let (other, other_email) = parity::caller(&pool, "grant-other").await;
    let created = create(&owner, &pool, &owner_email, "Grant target", None).await;
    let id = created["resource"]["id"].as_str().unwrap();
    let other_profile_id: Uuid = sqlx::query_scalar("SELECT id FROM kb_profiles WHERE email = $1")
        .bind(&other_email)
        .fetch_one(&pool)
