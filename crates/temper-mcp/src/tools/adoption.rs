//! Resource adoption tool — one bounded, resumable corpus-adoption step.
//!
//! Mirrors the HTTP endpoint `POST /api/resources/adopt` (`temper-api/src/handlers/adopt.rs`)
...
use temper_core::types::adoption::{AdoptScope, DEFAULT_ADOPT_LIMIT};
...
/// MCP input for resource_adopt — one bounded, resumable adoption step.
#[derive(Debug, Deserialize, JsonSchema)]
pub struct ResourceAdoptInput {
    /// What this invocation covers — exactly one arm.
    pub scope: AdoptTarget,
...
/// Run one corpus-adoption step over the tool's scope arm.
///
/// CLI equivalent: `temper admin adopt --resource|--context|--all`.
pub async fn resource_adopt(
    svc: &TemperMcpService,
    input: ResourceAdoptInput,
    ...
    let scope = match input.scope {
        AdoptTarget::Resource => {
            ...
            AdoptScope::Resource(Uuid::from(resource))
        }
        AdoptTarget::Context => {
            ...
            AdoptScope::Context(Uuid::from(context))
        }
        AdoptTarget::All => AdoptScope::All,
    };

    let cmd = AdoptResources {
        scope,
        dry_run: input.dry_run,
        limit: input.limit.unwrap_or(DEFAULT_ADOPT_LIMIT),
    ...
    let out = backend
        .adopt_resources(cmd)
        .await
        .map_err(|e| map_err(e, "resource_adopt"))?;
...
mod tests {
    ...
    #[test]
    fn resource_adopt_input_deserializes_all_three_scope_arms() {
        let one: ResourceAdoptInput = serde_json::from_value(serde_json::json!({
            "scope": "resource",
            "resource": "019e84ab-26ba-7560-9d34-c60d74a9fbe2",
            "dry_run": true
        }))
        .unwrap();
        assert!(matches!(one.scope, AdoptTarget::Resource));
        assert_eq!(
        ...
        let many: ResourceAdoptInput = serde_json::from_value(serde_json::json!({
            "scope": "context",
            "context": "@me/temper",
            "limit": 25
        }))
        .unwrap();
        assert!(matches!(many.scope, AdoptTarget::Context));
        assert_eq!(many.context.as_deref(), Some("@me/temper"));
        assert_eq!(many.limit, Some(25));
        assert!(!many.dry_run, "dry_run defaults to act");

        let everywhere: ResourceAdoptInput = serde_json::from_value(serde_json::json!({
            "scope": "all",
            "after_id": "019e84ab-26ba-7560-9d34-c60d74a9fbe3"
        }))
        .unwrap();
        assert!(matches!(everywhere.scope, AdoptTarget::All));
    ...
    #[test]
    fn resource_adopt_schema_inlines_its_scope_enum() {
        let schema = rmcp_schema_for::<ResourceAdoptInput>();
        assert!(
