...
impl TemperMcpService {
    ...
    #[tool(
        description = "Read one content block by address — the three-state resolution. `state` is always named: `live` returns the block's identity, its chunks' identities (structure and hashes, never prose), and its provenance rows; `folded` means a re-partition folded it away and returns its persisted attribution history plus `disposition` — where the content went (`located` names the absorber/carried successor blocks, only those you can read), `content_gone`, or `unrecorded` (the ledger does not carry the mapping). A block that does not exist or is not visible to you is a not-found error. Address a folded successor by calling this again with its block_id."
    )]
    async fn get_block(
        &self,
