//! `AdminClient::adopt` — the corpus-adoption step.
//!
...
use temper_core::types::adoption::{
    AdoptDeclined, AdoptOutcome, AdoptReceipt, AdoptRequest, AdoptResourceOutcome, AdoptSummary,
};
    ...
    AdoptReceipt {
        ...
        outcomes: vec![
            AdoptResourceOutcome {
                ...
                outcome: AdoptOutcome::Reblocked {
                    event: Uuid::now_v7(),
                },
            },
            AdoptResourceOutcome {
                resource: Uuid::now_v7(),
                outcome: AdoptOutcome::Declined(AdoptDeclined::Denied),
            },
        ],
...
#[tokio::test]
async fn adopt_posts_the_resource_scope_body_to_resources_adopt() {
    ...
    let body = AdoptRequest {
        scope: temper_core::types::adoption::AdoptScope::Resource(resource_id),
        dry_run: true,
        limit: Some(50),
...
#[tokio::test]
async fn adopt_all_scope_serializes_as_the_bare_string_and_receipt_round_trips() {
    let server = MockServer::start().await;
    let expected_receipt = receipt();
    ...
    let client = test_client(&server.uri());
    let body = AdoptRequest {
        scope: temper_core::types::adoption::AdoptScope::All,
        dry_run: false,
        limit: None,
        after_id: None,
    };
