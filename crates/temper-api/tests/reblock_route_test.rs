//! Route witnesses for `POST /api/resources/reblock` — the corpus re-blocking wire surface.
//!
...
//! decline taxonomy, ledger effects) is covered by temper-services' `reblock_service_test` and
...
use sqlx::PgPool;
use temper_core::types::reblock::{ReblockRequest, ReblockScope};
...
/// The reblock request body, built through the typed request DTO — the same construction a typed
/// client performs, so the DTO's serde contract is what the route actually receives.
fn reblock_body(scope: ReblockScope, dry_run: bool, limit: Option<i64>) -> serde_json::Value {
    serde_json::to_value(&ReblockRequest {
        scope,
        dry_run,
        limit,
        after_id: None,
    })
    .expect("ReblockRequest serializes")
}
...
/// the reblock tooling faces it). Returns the new resource id.
async fn fire_conformed_resource(
    ...
    fire(
        ...
        SeedAction::ResourceCreate {
            title,
            origin_uri: &format!("temper://reblock-route/{title}"),
            resource_id: None,
...
/// An unauthenticated POST to the reblock route is refused at the transport — the route lives on
/// the gated chain, so `require_auth` answers before any handler or backend code runs.
#[sqlx::test(migrator = "temper_api::MIGRATOR")]
async fn unauthenticated_reblock_is_unauthorized(pool: PgPool) {
    let app = common::setup_test_app(pool).await;

    let resp = app
        .client
        .post(app.url("/api/resources/reblock"))
        .json(&reblock_body(ReblockScope::All, true, None))
        .send()
        .await
        .expect("reblock request failed");

    assert_eq!(
        resp.status().as_u16(),
        401,
        "unauthenticated reblock must be 401; body: {}",
...
async fn non_admin_all_scope_is_refused_through_the_route(pool: PgPool) {
    ...
    let email = format!("reblock-nonadmin-{}@example.com", Uuid::new_v4());
    ...
    assert_eq!(
        resp.status().as_u16(),
        403,
        "non-admin all-scope reblock must be 403; body: {}",
        resp.text().await.unwrap_or_default()
    );
}
...
async fn dry_run_context_survey_returns_a_well_formed_receipt(pool: PgPool) {
    ...
    let email = format!("reblock-owner-{}@example.com", Uuid::new_v4());
    ...
    let resp = app
        ...
        .post(app.url("/api/resources/reblock"))
        .header("Authorization", format!("Bearer {token}"))
        .json(&reblock_body(ReblockScope::Context(context_id), true, None))
        .send()
        .await
        .expect("reblock request failed");
    ...
    assert_eq!(status, 200, "survey reblock must be 200; body: {body}");
...
async fn non_positive_limit_is_a_bad_request(pool: PgPool) {
    ...
    let email = format!("reblock-limit-{}@example.com", Uuid::new_v4());
    let (profile_id, context_id) =
        common::fixtures::create_test_profile_with_context(&app.pool, &email).await;
    let token = common::generate_test_jwt(&format!("test|{profile_id}"), &email);
    ...
    let resp = app
        ...
        .header("Authorization", format!("Bearer {token}"))
        .json(&reblock_body(
            ReblockScope::Context(context_id),
            true,
            Some(0),
        ))
        .send()
