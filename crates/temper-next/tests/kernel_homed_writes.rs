...
use temper_next::payloads::EdgePolarity;
use temper_next::scenario::{loader, model::Seed};
use temper_next::substrate;
use temper_next::writes::{self, KernelCreateParams, KernelEdgeParams};
...
async fn create_kernel_resource_homes_to_cogmap_with_facet_and_edge() {
    ...
    let slice = temper_next::readback::kernel_slice(&pool, cogmap.uuid())
        .await
        .unwrap();
