...
//! never authoritative.
//!
//! Lives here rather than in temper-workflow because [`ResourceView`] —
//! the one shape a resource has — derives its `ref` through
//! [`decorated_ref`], and `ResourceView` is read back by temper-substrate,
//! the layer *below* temper-workflow. `temper_workflow::operations`
//! re-exports every item here at its incumbent path.
//!
...
/// ref / filename. The output is always `validate_slug`-conformant (ASCII
/// lowercase alphanumerics separated by single hyphens) — this is what lets a
...
///
/// `validate_slug` itself lives one layer up, in
/// `temper_workflow::operations::validate_slug`, so the conformance property is
/// asserted there (`temper-workflow/src/operations/refs.rs`) rather than here —
/// the assertion crosses the crate boundary in the only direction that is
/// acyclic.
///
...
mod tests {
    ...
    ///
    /// The `validate_slug`-conformance half of this property lives in
    /// `temper-workflow/src/operations/refs.rs`, where the validator is reachable;
    /// the transliteration table itself is asserted here, beside the fold.
    #[test]
    fn sluggify_transliterates_non_ascii_to_ascii() {
        assert_eq!(
