...
fn run(cli: Cli, output_format: OutputFormat) -> temper_cli::error::Result<()> {
    match cli.command {
            ...
            match action {
                ...
                ResourceAction::Update {
                    ...
                    let params = temper_cli::commands::resource::UpdateParams {
                        ...
                        open_meta: open_meta.as_deref(),
                        open_meta_add: open_meta_add.as_deref(),
                        stage: stage.as_deref(),
                        mode: mode.as_deref(),
                        effort: effort.as_deref(),
                        seq,
        ...
        Commands::Query { plan, check } => {
            ...
            commands::query_cmd::run(plan.as_deref(), check, stdin_is_tty, output_format)
        }
        Commands::Search {
            query,
            context,
