...
pub enum ResourceAction {
    ...
    Create {
        ...
        /// Supported formats: md/markdown, txt/text, html/htm, pdf (text-layer PDFs only —
        /// a scanned or image-only PDF extracts nothing, as there is no OCR). Mutually
        /// exclusive with --body. http(s) URLs are fetched; a local file may be given as a
        /// plain path or a file:// URI.
    ...
    ///
    /// States `live`, `folded`, or `absent` BY NAME in the envelope's `state` field.
    /// `live` carries the block's identity, its chunks' identities (structure and
