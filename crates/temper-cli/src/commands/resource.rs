...
/// `DomainEvent::VaultFileWritten` is present in the output's events)
/// or "(no slug)" otherwise.
///
/// Generic over the output value type `T`. Used for `CommandOutput<()>`
/// (delete) and similar. For `CommandOutput<ResourceRow>`, prefer
/// `render_write_success_value` so the slug fallback works.
///
/// Mode-implicit via event presence: local mode emits `VaultFileWritten`
/// (rel_path goes into the message); cloud mode does not (slug fallback).
/// Surfaces don't match on `VaultState`.
#[expect(dead_code, reason = "wired in Task 9 onward")]
pub(crate) fn render_write_success<T>(
    verb: &str,
    output: &temper_core::operations::CommandOutput<T>,
) {
    output::success(render_write_success_to_string(verb, output));
}

/// Test-friendly core of `render_write_success` — returns the string that
/// would be printed. Generic over `T`; use `render_write_success_value`
/// for `CommandOutput<ResourceRow>` so the slug fallback applies.
pub(crate) fn render_write_success_to_string<T>(
    ...
    match rel_path {
        ...
        None => format!("{verb}: (no slug)"),
    }
}

/// Specialization for `CommandOutput<ResourceRow>` — uses `value.slug`
/// as the fallback when no `VaultFileWritten` event is present.
///
/// Use this from create/update surfaces; use the generic
/// `render_write_success_to_string` for delete (where value is `()`).
...
mod render_write_success_tests {
    ...
    use super::render_create_output_tests::make_resource_row;
    use super::{render_write_success_to_string, render_write_success_value};

    #[test]
    fn render_write_success_value_uses_rel_path_when_vault_file_written_present() {
        let row = make_resource_row("the-slug", "task", "Title", "temper");
        ...
        let s = render_write_success_value("Updated", &output);
        assert_eq!(s, "Updated: @me/temper/task/the-slug.md");
    }

    #[test]
    fn render_write_success_value_uses_slug_when_no_vault_file_written() {
        let row = make_resource_row("the-slug", "task", "Title", "temper");
        let output = CommandOutput {
            value: row,
    ...
    #[test]
    fn render_write_success_value_falls_back_to_no_slug_marker_when_slug_missing() {
        let mut row = make_resource_row("ignored", "task", "Title", "temper");
        row.slug = None;
        let output = CommandOutput {
            value: row,
            events: vec![],
        };
        let s = render_write_success_value("Created", &output);
        assert_eq!(s, "Created: (no slug)");
    }

    #[test]
    fn render_write_success_generic_uses_rel_path_for_command_output_unit() {
        let output = CommandOutput::<()> {
            value: (),
            events: vec![DomainEvent::VaultFileWritten {
                path: "@me/temper/task/x.md".to_string(),
            }],
        };
        let s = render_write_success_to_string("Deleted", &output);
        assert_eq!(s, "Deleted: @me/temper/task/x.md");
    }

    #[test]
    fn render_write_success_generic_falls_back_for_command_output_unit() {
        let output = CommandOutput::<()> {
            value: (),
            events: vec![],
        };
        let s = render_write_success_to_string("Deleted", &output);
        assert_eq!(s, "Deleted: (no slug)");
    }
}
