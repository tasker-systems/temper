...
//! `UPDATE_SCHEMA=1 cargo test -p temper-substrate --features scenario-schema`.

use temper_substrate::scenario::model::{Scenario, Seed};
...
fn access_scenario_json_schema_matches_snapshot() {
    let schema = schemars::schema_for!(temper_substrate::scenario::access::model::AccessScenario);
    let rendered = serde_json::to_string_pretty(&schema).unwrap() + "\n";
